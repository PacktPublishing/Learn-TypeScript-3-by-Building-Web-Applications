...
import {MusicService} from '@/services/music-service.intf';
import {MusicServiceImpl} from '@/services/music-service';
...
describe('music service', () => {
  ...
  describe('initialization', () => {
    it('should succeed if the URL is provided and valid', () => {
      expect(new MusicServiceImpl('https://foo', axios)).toBeInstanceOf(MusicServiceImpl);
      expect(new MusicServiceImpl('https://foo/', axios)).toBeInstanceOf(MusicServiceImpl);
      expect(new MusicServiceImpl('http://foo', axios)).toBeInstanceOf(MusicServiceImpl);
      expect(new MusicServiceImpl('http://foo/', axios)).toBeInstanceOf(MusicServiceImpl);
      expect(new MusicServiceImpl('HTTP://foo/', axios)).toBeInstanceOf(MusicServiceImpl);
      expect(new MusicServiceImpl('HTTPS://foo/', axios)).toBeInstanceOf(MusicServiceImpl);
    });
  ...
  describe('findArtists', () => {
    ...
    it('should return artists when resultset is not empty', (done: () => {}) => {
      axiosMock.onGet().reply(200, {
          message: {
            header: {status_code: 200, execute_time: 0.05787992477417, available: 137}, body: {
              artist_list: [{
                artist: {
                  artist_id: 1039,
                  ...
                  artist_comment: '',
                  artist_country: 'GB',
                  artist_alias_list: [{artist_alias: '\u30b3\u30fc\u30eb\u30c9\u30d7\u30ec\u30a4'}, {artist_alias: 'ku wan yue dui'}, {artist_alias: 'The Coldplay'}, {artist_alias: 'Cold Play'}],
                  artist_rating: 94,
  ...
  describe('findSongs', () => {
    ...
    it('should return songs when resultset is not empty', (done: () => {}) => {
      axiosMock.onGet().reply(200, {
          message: {
            header: {status_code: 200, execute_time: 0.058778047561646, available: 4}, body: {
              track_list: [{
                track: {
                  track_id: 69783725,
                  ...
                  track_share_url: 'https:\/\/www.musixmatch.com\/lyrics\/Guarana-Goal\/Viva-La-Vida-Loca?utm_source=application&utm_campaign=api&utm_medium=undefined%3A1409618355445',
                  track_edit_url: 'https:\/\/www.musixmatch.com\/lyrics\/Guarana-Goal\/Viva-La-Vida-Loca\/edit?utm_source=application&utm_campaign=api&utm_medium=undefined%3A1409618355445',
                  restricted: 0,
  ...
  describe('findLyrics', () => {
    ...
    it('should fail if no song is given', () => {
      expect(() => {
        sut.findLyrics(null as unknown as Song);
      }).toThrow();
    });

    it('should fail if the song is not supposed to have lyrics', () => {
      expect(() => {
        sut.findLyrics(new Song('foo', 'bar', 'baz', false, []));
      }).toThrow();
    });
    ...
    it('should fail if no song lyrics response is received', (done: () => {}) => {
      ...
      sut.findLyrics(new Song('foo', 'bar', 'baz', true, [])).subscribe({
        next: () => {
    ...
    it('should return lyrics when resultset is not empty', (done: () => {}) => {
      axiosMock.onGet().reply(200, {
        message: {
          header: {status_code: 200, execute_time: 0.0089390277862549}, body: {
            lyrics: {
              lyrics_id: 1665915,
              explicit: 1,
