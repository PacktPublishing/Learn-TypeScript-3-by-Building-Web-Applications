...
describe('song lyrics', () => {
  ...
  it('should not accept undefined lyrics', () => {
    expect(() => {
      new SongLyrics('foo', null as unknown as string, true, '');
    }).toThrow();

    expect(() => {
      new SongLyrics('foo', undefined as unknown as string, true, '');
    }).toThrow();
  });

  it('should not accept empty lyrics', () => {
    expect(() => {
      new SongLyrics('foo', '', true, '');
    }).toThrow();
